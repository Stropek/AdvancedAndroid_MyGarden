...
public class PlantWateringService extends IntentService {
     ...
     * @see IntentService
     */
    public static void startActionWaterPlant(Context context, String plantId) {
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            if (ACTION_WATER_PLANT.equals(action)) {
                long plantId = intent.getLongExtra(EXTRA_PLANT_ID, 0);
    ...
    private void handleActionWaterPlants(long plantId) {
        Uri singlePlantUri = BASE_CONTENT_URI.buildUpon()
    ...
    private void handleActionUpdatePlantWidgets() {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            hideWaterDrop = timeNow - wateredAt < PlantUtils.MIN_AGE_BETWEEN_WATER;
        }
        AppWidgetManager appWidgetManager = AppWidgetManager.getInstance(this);
        int[] appWidgetIds = appWidgetManager.getAppWidgetIds(new ComponentName(this, PlantWidgetProvider.class));

        //Now update all widgets
        PlantWidgetProvider.updatePlantWidgets(this, appWidgetManager, imgRes, appWidgetIds, plantId, hideWaterDrop);
    }
}
